...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
    *  This is same as splitting this and inserting each of this to that.
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
      ...
      val currentAcc = if (p(elem)) leftAcc.incl(elem) else leftAcc
      right.filterAcc(p, currentAcc)
    }
  ...
  override def mostRetweeted: Tweet = {
    ...
    val leftRightMost = if (leftMost.retweets > rightMost.retweets) leftMost else rightMost
    if (leftRightMost.retweets > elem.retweets) leftRightMost else elem
  }
